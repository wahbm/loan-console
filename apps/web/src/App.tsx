...
function PrepaymentPanel({ loan, asOf }: { loan: LoanCase; asOf: string }) {
  ...
  const [interestAmounts, setInterestAmounts] = useState<Record<string, string>>(() => Object.fromEntries(loan.components.map((component) => [component.id, ""])));
  const [saved, setSaved] = useState(false);
  const input = useMemo<PrepaymentInput>(() => ({ date, strategy, components: loan.components.map((component) => ({ componentId: component.id, amount: amounts[component.id] || "0", interestAmount: interestAmounts[component.id] || "0" })) }), [amounts, date, interestAmounts, loan.components, strategy]);
  ...
  return <section className="panel prepayment-panel"><div className="panel-heading"><div><span className="eyebrow">WHAT IF / PREPAYMENT</span><h3>提前还款模拟</h3></div><span className="step-badge">不修改原贷款</span></div><div className="prepayment-form"><label>提前还款日期<input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label><label>方案<select value={strategy} onChange={(event) => setStrategy(event.target.value as PrepaymentInput["strategy"])}><option value="reduce_term">保持月供，缩短期限</option><option value="reduce_payment">保持期限，降低月供</option></select></label>{loan.components.map((component) => <React.Fragment key={component.id}><label>{componentLabel(component.componentType)}提前还款本金<input placeholder="0.00" inputMode="decimal" value={amounts[component.id] ?? ""} onChange={(event) => setAmounts((current) => ({ ...current, [component.id]: event.target.value }))} /></label><label>{componentLabel(component.componentType)}额外期间利息<input placeholder="0.00" inputMode="decimal" value={interestAmounts[component.id] ?? ""} onChange={(event) => setInterestAmounts((current) => ({ ...current, [component.id]: event.target.value }))} /></label></React.Fragment>)}<button className="primary-button" onClick={() => preview.mutate()} disabled={preview.isPending}>{preview.isPending ? "计算中…" : "查看前后变化"}</button></div><p className="form-footnote">本金会冲减贷款余额；额外期间利息只计入本次提前还款成本，不会冲减本金。金额应以银行结算单为准。</p><ErrorBanner error={preview.error ?? save.error} />{preview.data && <div className="comparison"><ComparisonMetric label="计划剩余利息" before={money(preview.data.before.metrics.remainingInterest)} after={money(preview.data.after.metrics.remainingInterest)} saving={`计划减少 ${money(preview.data.savedInterest)}`} /><ComparisonMetric label="提前还款期间利息" before="—" after={money(preview.data.prepaymentInterest)} saving="不计入本金" /><ComparisonMetric label="预计总利息净节省" before="—" after={money(preview.data.netSavedInterest)} saving={`计划利息减少 ${money(preview.data.totalInterestSaved)}`} /><ComparisonMetric label="剩余期数" before={`${preview.data.before.metrics.remainingPeriods} 期`} after={`${preview.data.after.metrics.remainingPeriods} 期`} saving={preview.data.savedPeriods > 0 ? `缩短 ${preview.data.savedPeriods} 期` : "期限不变"} /><ComparisonMetric label="截至所选日期的下期还款" before={money(preview.data.before.metrics.nextPayment)} after={money(preview.data.after.metrics.nextPayment)} saving={preview.data.firstPaymentReduction !== "0.00" ? `减少 ${money(preview.data.firstPaymentReduction)}` : "该期金额不变"} />{preview.data.effectivePayments?.map((item) => <React.Fragment key={item.componentId}><ComparisonMetric label={`${componentLabel(loan.components.find((component) => component.id === item.componentId)!.componentType)}生效后首期还款`} before={money(item.beforePayment)} after={money(item.afterPayment)} saving={item.paymentDate ? `还款日 ${item.paymentDate}` : "该分项已结清"} /><p className="muted-copy">{item.boundaryDate} 正常还款后扣减提前还款本金；{item.paymentDate ? `新计划从 ${item.paymentDate} 开始。` : "之后无需还款。"}</p></React.Fragment>)}<div className="comparison-actions"><button className="secondary-button" onClick={() => save.mutate()} disabled={save.isPending || saved}>{saved ? "方案已保存" : "保存这次方案"}</button><span>计算版本 {"1.1"}</span></div></div>}</section>;
}
